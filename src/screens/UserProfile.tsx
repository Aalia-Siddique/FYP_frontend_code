...
const UserProfileScreen = ({ route, navigation }) => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch(`http://192.168.108.30:5165/api/Users/GetUsersById/${userId}`);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.profileHeader}>
        <Image source={{ uri: `http://192.168.108.30:5165/${user.userImageName}` }} style={styles.userImage} />
      ...
      <View style={styles.detailsContainer}>
        ...
        <Image source={{ uri: `http://192.168.108.30:5165/${user.cnicImageName}` }} style={styles.extraImage1} />
      </View>

      <View style={styles.detailsContainer}>
        <Text style={styles.sectionTitle}>License and Certificate</Text>
        <TouchableOpacity onPress={() => Linking.openURL(`http://192.168.108.30:5165/${user.certificateImageName}`)}>
          <Image source={{ uri: `http://192.168.108.30:5165/${user.certificateImageName}` }} style={styles.extraImage} />
        </TouchableOpacity>
        <TouchableOpacity 
          style={styles.downloadButton} 
          onPress={() => Linking.openURL(`http://192.168.108.30:5165/${user.certificateImageName}`)}
        >
