...
const ReviewsScreen = ({ route }) => {
    ...
    const fetchFeedbacks = async () => {
        try {
            const response = await axios.get(`http://192.168.100.22:5209/api/Feedback/TargetFeedback/${userId}`);
    ...
    const submitFeedback = async () => {
        ...
        try {
            const response = await axios.post('http://192.168.100.22:5209/api/Feedback/PostFeedback', {
                targetId: userId,
                senderId: senderId,
                rating,
                comment,
