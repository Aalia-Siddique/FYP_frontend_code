...
const addSection = (section: Section) => {
  setSections((prev) => [...prev, section]); // Add the new section
  setExpanded(false); // Close the expanded state (if applicable)
};

// Remove a section
const removeSection = (section: Section) => {
  setSections((prev) => prev.filter((item) => item.id !== section.id)); // Remove based on id
};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.0.101:5140/api/JobPost', {
        method: 'POST',
        headers: {
